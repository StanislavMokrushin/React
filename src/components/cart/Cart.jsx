...
const Cart = (props) => {
    ...
    const onAddOverlay = async (obj)=>{
        try{
            ...
            if(findOverlay)
            {
            axios.delete(`https://6399f45316b0fdad774f527f.mockapi.io/cart/${findOverlay.id}`)
            props.setOverlayItems((over) => over.filter(item => item.myId !== obj.myId))
            }
            else{
              const {data} = await axios.post('https://6399f45316b0fdad774f527f.mockapi.io/cart', obj)
    ...
    const onAddFav = async (obj)=>{
        ...
        try{
            const findFavorites = props.favorites.find(objFav => objFav.myId === obj.myId)
             if(findFavorites){
            axios.delete(`https://6399f45316b0fdad774f527f.mockapi.io/favorites/${findFavorites.id}`)
            props.setFavorites((over) => over.filter(item => item.myId !== obj.myId))

        }
        else{
          const {data} = await  axios.post('https://6399f45316b0fdad774f527f.mockapi.io/favorites', obj)
